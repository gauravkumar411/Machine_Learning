...
def getWeightedDistanceRegression(trainData, testData, k):
# This function calculates the Class(Regression) value for the test instance
    ...
    nTor = 0# Initializing value of numerator to 0
    dTor = 0# Initializing value of denominator to 0
    
    neighbours = np.array(neighbours)# Converting list to numpy array
    
    for i in range(k):
        nTor += (1/(dist[indices[i]]**2)) * (neighbours[i,-1])
        # Calculating numerator as per the below formula:
        #                       1
        #       nTor = --------------------------------------- x Class(regression) value of neighbour
        #               distance(neighbour, test instance)**2
        #
        dTor += 1/(dist[indices[i]]**2)
        # Calculating denominator as per the below formula:
        #                       1
        #       dTor = --------------------------------------
        #               distance(neighbour, test instance)**2
        #
    
    result = nTor/dTor# Dividing numerator and denominator
...
# This is the main function
    ...
    for k in range(1, 20):
        ...
        for x in range(len(testData)):
            result = getWeightedDistanceRegression(trainData, testData[x], k)
            #Calling getWeightedDistanceRegression function to calculate the Class(Regression) value for each test instance
    ...
    plt.title('k-NN Distacne Weighted Regression Graph')# Setting title of graph
